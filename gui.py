...
class Gui(wx.Frame):
    ...
    def add_menu_bar(self):
        ...
        self.insert_vertex = self.graphMenu.Append(-1, "&Inserir Vértice...\tCtrl-A", "Insere um vértice no grafo!")
        self.insert_edge = self.graphMenu.Append(-1, "&Inserir Aresta...\tCtrl-B", "Insere uma aresta no grafo!")
        self.remove_vertex = self.graphMenu.Append(-1, "&Remover Vértice...\tCtrl-C", "Remove um vértice do grafo!")
        self.remove_edge = self.graphMenu.Append(-1, "&Remover Aresta...\tCtrl-D", "Remove uma aresta do grafo!")
        self.check_adjacency = self.graphMenu.Append(-1, "&Verificar Adjacência\tCtrl-E", "Verificar se dois vértices são adjacentes!")
        self.return_edge_element = self.graphMenu.Append(-1, "&Retornar Elemento da Aresta\tCtrl-F", "Retorna o elemento da aresta!")
        self.return_vertex_element = self.graphMenu.Append(-1, "&Retornar Elemento do Vértice\tCtrl-G", "Retorna o elemento do vértice!")
        ...
        self.check_planar_graph = self.graphMenu.Append(-1, "&Verificar Planaridade...\tCtrl-H", "Verifica se o grafo é planar!")
        self.breadth_search = self.graphMenu.Append(-1, "&Busca em Largura...\tCtrl-I", "Efetua a busca em largura no grafo!")
        self.depth_search = self.graphMenu.Append(-1, "&Busca em Profundidade...\tCtrl-J", "Efetua a busca em profundidade no grafo!")
        self.prim = self.graphMenu.Append(-1, "&PRIM...\tCtrl-K", "Executa o algoritmo de PRIM!")
        self.graphMenu.AppendSeparator()
        self.green_coloring = self.graphMenu.Append(-1, "&Coloração Gulosa...\tCtrl-L", "Executa o algorimo de coloração gulosa!")
        self.floyd = self.graphMenu.Append(-1, "&Floyd...\tCtrl-M", "Executa o algorimo floyd!")
        self.dijkstra = self.graphMenu.Append(-1, "&Dijkstra...\tCtrl-N", "Executa o algorimo dijkstra!")
        self.a_star = self.graphMenu.Append(-1, "&A*...\tCtrl-O", "Executa o algoritmo A*")
        self.graphMenu.AppendSeparator()
        self.exitItem = self.graphMenu.Append(wx.ID_EXIT)
        self.editMenu = wx.Menu()
        self.zoom_in = self.editMenu.Append(-1, "&Zoom In...\tCtrl-=", "Aumentar o Zoom")
        self.zoom_out = self.editMenu.Append(-1, "&Zoom Out...\tCtrl--", "Diminuir o Zoom")
        self.change_directed_status = self.editMenu.Append(-1, "&Directed...\tCtrl-P", "Alterna status de Dirigido")
        # ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        # menu bar
