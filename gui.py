...
class Gui(wx.Frame):
    ...
    def add_menu_bar(self):
        ...
        self.insert_vertex = self.graphMenu.Append(-1, "&Inserir Vértice...\tCtrl-1", "Insere um vértice no grafo!")
        self.insert_edge = self.graphMenu.Append(-1, "&Inserir Aresta...\tCtrl-2", "Insere uma aresta no grafo!")
        self.remove_vertex = self.graphMenu.Append(-1, "&Remover Vértice...\tCtrl-3", "Remove um vértice do grafo!")
        self.remove_edge = self.graphMenu.Append(-1, "&Remover Aresta...\tCtrl-4", "Remove uma aresta do grafo!")
        self.return_edge_element = self.graphMenu.Append(-1, "&Retornar Elemento da Aresta\tCtrl-5", "Retorna o elemento da aresta!")
        self.return_vertex_element = self.graphMenu.Append(-1, "&Retornar Elemento do Vértice\tCtrl-6", "Retorna o elemento do vértice!")
        self.return_vertices_references_from_edge = self.graphMenu.Append(-1, "&Retornar a Referência dos Vértices da Aresta\tCtrl-7", "Retorna a referência dos dois vértices de uma aresta!")
        self.check_adjacency = self.graphMenu.Append(-1, "&Verificar Adjacência\tCtrl-J", "Verificar se dois vértices são adjacentes!")
        self.check_planar_graph = self.graphMenu.Append(-1, "&Verificar Planaridade...\tCtrl-P", "Verifica se o grafo é planar!")
        ...
        self.a_star = self.graphMenu.Append(-1, "&A*...\tCtrl-8", "Executa o algoritmo A*")
        self.graphMenu.AppendSeparator()
        self.tsp = self.graphMenu.Append(-1, "&Problema do Caixeiro Viajante\tCtrl-T", "Executa a solução para o problema do caixeiro viajante.")
        self.graphMenu.AppendSeparator()
        ...
        self.change_directed_status = self.editMenu.Append(-1, "&Directed...\tCtrl-O", "Alterna status de Dirigido")
        # ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        # menu bar
        ...
        self.Bind(wx.EVT_MENU, self.evt.on_tsp, self.tsp)
